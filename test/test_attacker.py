...
class TestAttacker(unittest.TestCase):
    ...
    def test_add_two_prob_model_2(self):
        ...
        self.assertEqual(np.array_equal(model3.fields_array, np.array([[0,0],[1,0],[0,1],[1,1]])), True)
    ...
    def test_create_attacker_model1(self):
        ...
        self.assertIn(('black', 'non-hispanic', 'NC'), keys)
        self.assertIn(('black', 'non-hispanic', 'TN'), keys)
        self.assertIn(('white', 'hispanic', 'NC'), keys)
        self.assertIn(('white', 'hispanic', 'TN'), keys)
        self.assertEqual(attack1.model[('black', 'non-hispanic', 'NC')].fields, ['state', 'race', 'gender', 'age'])
        self.assertEqual(np.array_equal(attack1.model[('black', 'non-hispanic', 'NC')].fields_array, np.array([[0,0,0,0],[1,1,1,1]])), True)
        self.assertEqual(np.array_equal(attack1.model[('black', 'non-hispanic', 'NC')].prob_list, np.array([0.5,0.5])), True)
        #    print (value.fields)
